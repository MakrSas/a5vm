...
Сборка полностью проходит в CI и даёт готовый к установке `A5VM.app`
(около 14 МБ): armv7, минимальная версия iOS 6.0, зависимости — только
системные библиотеки устройства.

Проверено на артефакте:

* обе части — `armv7`, обе несут legacy-команду `LC_VERSION_MIN_IPHONEOS`
  с минимумом 6.0 (`LC_BUILD_VERSION` dyld на iOS 6 не понимает);
* в `libqemu-system-i386.dylib` нет ни одной ссылки на пути сборочной
  машины — только `libSystem`, `CoreFoundation`, `libz`, `libiconv`,
  `libresolv`;
* все 10 символов моста `a5_qemu_*` экспортируются;
* `A5VM` не слинкован с QEMU — библиотека грузится через `dlopen`.

**Что ещё не проверено:** запуск на настоящем устройстве. До первого пуска
неизвестно, даёт ли ядро с этим jailbreak исполняемую память для TCG, и
верно ли угадан порядок байтов в кадре. Порядок проверки — в
