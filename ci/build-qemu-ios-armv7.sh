...
# util/cacheinfo.c's constructor (runs at dylib-load time, before
# main()) crashed on the real iPhone 4S: `assert((isize & (isize - 1))
# == 0)` failed. A device smoke test with temporary diagnostic prints
# (see git history) found why: sysctlbyname("hw.cachelinesize", &size,
# &len, NULL, 0) returns 0 (success) but with len=0 -- it wrote nothing
# into `size` -- yet the upstream code only checks the return code, not
# whether len actually came back as sizeof(size), so it trusts
# whatever garbage was already on the stack (observed: 804402612,
# obviously not a power of two). `sysctl hw.cachelinesize` from a shell
# reports a normal 32 on the same device even as the unprivileged
# mobile user, so the target itself is fine; something about calling
# sysctlbyname this early, from a dylib's own __attribute__((constructor))
# invoked directly by dyld before the rest of the process has finished
# starting, doesn't behave the same way as a fully-started process
# calling it later. Rather than chase exactly why dyld-constructor-time
# is special on this old XNU, fix the actual bug: don't trust a
# "successful" sysctlbyname that didn't fill the buffer it claims to
# have filled. This falls through correctly to fallback_cache_info's
# existing generic 64-byte default (a value QEMU's own comment already
# treats as an acceptable guess, used only as a cache-flush/atomics
# granularity hint, not for correctness) instead of crashing.
CACHEINFO_C="$QEMU_SOURCE/util/cacheinfo.c"
perl -0pi -e 's/if \(!sysctlbyname\("hw\.cachelinesize", &size, &len, NULL, 0\)\) \{/if (!sysctlbyname("hw.cachelinesize", \&size, \&len, NULL, 0) \&\& len == sizeof(size)) {/' "$CACHEINFO_C"
grep -q 'len == sizeof(size)' "$CACHEINFO_C" || { echo "Failed to patch sys_cache_info in $CACHEINFO_C" >&2; exit 1; }
